...
| `create_mock_order`（Bifu 专有） | 通过 | 待开通 Sandbox 账户权限 | 独立专有方法固定发送 `SANDBOX_MARKET`；新公网 Key 的私有只读认证已通过，但写入返回 `4006 SANDBOX_MARKET requires Sandbox account`，请求未产生订单或成交 |
...
实现，当前只缺平台为公网 Key 对应账户开通 Sandbox/MockTrade 权限后的在线成交闭环，之后才形成第一部分
领导汇报。
...
- 新账号复查：平台最初提供的内网 Key 在公网开发 URL 的私有请求上返回 HTTP 401、
  ...
  写入失败后的只读复核显示当前挂单、历史订单和成交均为 0。当前只需平台把该公网 Key 对应账户
  开通 Sandbox/MockTrade 权限，或提供已经开通该权限的公网账户；无需修改适配器签名和请求结构。
